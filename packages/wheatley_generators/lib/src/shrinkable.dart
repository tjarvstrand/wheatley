...
class Shrinkable<T> {
  ...
  Shrinkable._(this._value, Iterable<Shrinkable<T>> Function()? shrink) : shrink = shrink ?? (() => const []);
  ...
  /// - They are _simpler_ than this: The transitive hull is finite acyclic. If you would call [shrink] on all returned
  ...
  Iterable<Shrinkable<T>> Function() shrink;
  ...
  Shrinkable<T?> get nullable => Shrinkable<T?>(
        value,
        () => shrink().cast<Shrinkable<T?>>().followedBy([Shrinkable<T?>(null, () => [])]),
      );
  ...
  Iterable<T> get values sync* {
    yield value;
    for (final shrinkable in shrink()) {
